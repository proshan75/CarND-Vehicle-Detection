...
def imageCvtColor(color_space, image):
    if color_space != 'RGB':
        if color_space == 'HSV':
...
def extract_features(imgs, color_space='RGB', spatial_size=(32, 32),
    ...
    for file in imgs:
        ...
        feature_image = imageCvtColor(color_space, image)
        # collecting features in following order:
        #  spatial features
        #  histogram features
        #  HOG features
        ...
        # features collected in following order:
        #  spatial features
        #  histogram features
        #  HOG features
...
def single_img_features(img, color_space='RGB', spatial_size=(32, 32),
    ...
    feature_image = imageCvtColor(color_space, img)
...
def find_cars(img, color_space, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins):
    ...
    ctrans_tosearch = imageCvtColor(
        color_space=color_space, image=img_tosearch)
    if scale != 1:
    ...
    for xb in range(nxsteps):
        for yb in range(nysteps):
            ...
            subimg = cv2.resize(
                ctrans_tosearch[ytop:ytop+window, xleft:xleft+window], (96, 96))
